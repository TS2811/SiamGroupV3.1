...
import {
    Box, Typography, Paper, Grid, Chip, TextField, Button, Avatar,
            ...
            {loading ? (
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} md={3}>
                        <Paper sx={{ borderRadius: 3, maxHeight: 'calc(100vh - 280px)', overflow: 'auto' }} elevation={0}>
                    ...
                    <Grid item xs={12} md={9}>
                        {!selectedEmp ? (
                            ...
                            <>
                                ...
                                {summaryData && (
                                    <Paper sx={{ p: 1.5, mb: 1.5, borderRadius: 3 }} elevation={0}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
                                            ...
                                            <Box sx={{ display: 'flex', gap: 1, flex: 1, justifyContent: 'center' }}>
                                                {[
                                                    { label: 'วันทำงาน', value: summaryData.work_days, color: '#22C55E' },
                                                    { label: 'สาย', value: `${summaryData.late_count} ครั้ง`, color: '#F59E0B' },
                                                    { label: 'สายรวม', value: `${summaryData.late_minutes} นาที`, color: '#F59E0B' },
                                                    { label: 'OT', value: `${summaryData.ot_hours || 0} ชม.`, color: '#F97316' },
                                                ].map(item => (
                                                    <Box key={item.label} sx={{ textAlign: 'center', px: 1.5, py: 0.5, borderRadius: 2, bgcolor: `${item.color}10`, minWidth: 70 }}>
                                                        <Typography variant="subtitle2" fontWeight={700} sx={{ color: item.color, lineHeight: 1.3 }}>{item.value}</Typography>
