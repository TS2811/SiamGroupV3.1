...
export default function HrmLeaveMgmtPage() {
    ...
    return (
        <Box>
            ...
            {/* Leave Type Dialog */}
            <Dialog open={typeDialog} onClose={() => setTypeDialog(false)} maxWidth="sm" fullWidth slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
                ...
                <DialogContent dividers>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                        <Box sx={{ display: 'flex', gap: 2 }}>
                            <TextField sx={{ flex: 1 }} size="small" label="รหัส *" value={typeForm.code || ''} disabled={editingType} onChange={e => setTypeForm(f => ({ ...f, code: e.target.value }))} />
                            <TextField sx={{ flex: 2 }} size="small" label="ชื่อ (TH) *" value={typeForm.name_th || ''} onChange={e => setTypeForm(f => ({ ...f, name_th: e.target.value }))} />
                        </Box>
                        <Box sx={{ display: 'flex', gap: 2 }}>
                            <TextField sx={{ flex: 1 }} size="small" label="ชื่อ (EN)" value={typeForm.name_en || ''} onChange={e => setTypeForm(f => ({ ...f, name_en: e.target.value }))} />
                            <TextField sx={{ flex: 1 }} size="small" label="สิทธิ์สูงสุด (วัน)" type="number" value={typeForm.max_days || ''} onChange={e => setTypeForm(f => ({ ...f, max_days: e.target.value }))} />
                            <TextField sx={{ flex: 1 }} size="small" label="ล่วงหน้า (วัน)" type="number" value={typeForm.min_days_advance ?? ''} onChange={e => setTypeForm(f => ({ ...f, min_days_advance: e.target.value }))} />
                        </Box>
                        <Divider />
                        <Box sx={{ display: 'flex', gap: 2 }}>
            ...
            {/* Quota Dialog */}
            <Dialog open={quotaDialog} onClose={() => setQuotaDialog(false)} maxWidth="sm" fullWidth slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
                ...
                <DialogContent dividers>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 2 }}>
                            <TextField sx={{ flex: 1 }} size="small" label="ปี" type="number" value={quotaForm.year || quotaYear} onChange={e => setQuotaForm(f => ({ ...f, year: e.target.value }))} />
                            <TextField sx={{ flex: 1 }} size="small" label="สิทธิ์ (วัน) *" type="number" value={quotaForm.quota_days || ''} onChange={e => setQuotaForm(f => ({ ...f, quota_days: e.target.value }))} />
                            <TextField sx={{ flex: 1 }} size="small" label="ยกมา (วัน)" type="number" value={quotaForm.carried_days ?? 0} onChange={e => setQuotaForm(f => ({ ...f, carried_days: e.target.value }))} />
                        </Box>
                    </Box>
                </DialogContent>
                <DialogActions sx={{ p: 2 }}>
            ...
            <Dialog open={editQuotaDialog} onClose={() => setEditQuotaDialog(false)} maxWidth="xs" fullWidth slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
                <DialogTitle sx={{ fontWeight: 700 }}>
