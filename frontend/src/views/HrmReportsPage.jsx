...
import {
    ...
    TableHead, TableRow, Chip, Button, Grid, TextField, FormControl,
            ...
            {tab > 0 && (
                <Paper sx={{ p: 2.5, mb: 2, borderRadius: 3 }} elevation={0}>
                    <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        <TextField size="small" label="วันเริ่ม" type="date" InputLabelProps={{ shrink: true }}
                            value={filters.start_date} onChange={e => setFilters(f => ({ ...f, start_date: e.target.value }))}
                            sx={{ minWidth: 160 }} />
                        <TextField size="small" label="วันสิ้นสุด" type="date" InputLabelProps={{ shrink: true }}
                            ...
                            <Grid container spacing={2} sx={{ mb: 3 }}>
                                {[
                                    ...
                                    <Grid item xs={6} sm={3} key={item.label}>
                                        <Paper sx={{ p: 3, textAlign: 'center', borderRadius: 3, border: `2px solid ${item.color}20` }} elevation={0}>
